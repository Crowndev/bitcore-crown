...
Then, block signature (for more info, see core repo's `SignBlock` function in `src/mn-pos/stakeminer.cpp`):
...
Finally, stake pointer (for more info, see core repo's `src/mn-pos/stakepointer.h`):
